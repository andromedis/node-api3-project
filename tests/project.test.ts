...
describe('server.js', () => {
  describe('1 [GET] /api/users', () => {
    it('can get the correct number of users', async () => {
      let res: Response = await request(server).get('/api/users')
  ...
  describe('2 [GET] /api/users/:id', () => {
    it('can get the correct user', async () => {
      let res: Response = await request(server).get('/api/users/1')
    ...
    it('responds with a 404 if id does not exist', async () => {
      let res: Response = await request(server).get('/api/users/111')
  ...
  describe('3 [POST] /api/users', () => {
    ...
    it('responds with the newly created user', async () => {
      let res: Response = await request(server).post('/api/users').send({ name: 'foo' })
    ...
    it('responds with a 400 if missing name', async () => {
      let res: Response = await request(server).post('/api/users').send({ random: 'thing' })
  ...
  describe('4 [PUT] /api/users/:id', () => {
    ...
    it('responds with the newly updated user', async () => {
      let res: Response = await request(server).put('/api/users/1').send({ name: 'FRODO BAGGINS' })
      expect(res.body).toMatchObject({ id: 1, name: 'FRODO BAGGINS' })
    })
    it('responds with a 404 if user id does not exist', async () => {
      let res: Response = await request(server).put('/api/users/111').send({ name: 'FRODO BAGGINS' })
      expect(res.status).toBe(404)
    })
    it('responds with a 400 if missing name', async () => {
      let res: Response = await request(server).put('/api/users/1').send({ no: 'FRODO BAGGINS' })
      expect(res.status).toBe(400)
    })
    it('responds with the correct error message if missing name', async () => {
      let res: Response = await request(server).put('/api/users/1').send({ no: 'FRODO BAGGINS' })
  ...
  describe('5 [DELETE] /api/users/:id', () => {
    ...
    it('responds with the newly deleted user', async () => {
      let res: Response = await request(server).delete('/api/users/1')
      expect(res.body).toMatchObject({ id: 1, name: 'Frodo Baggins' })
    })
    it('responds with a 404 if user id does not exist', async () => {
      let res: Response = await request(server).delete('/api/users/111')
      expect(res.status).toBe(404)
    })
  })
  describe('6 [GET] /api/users/:id/posts', () => {
    it('gets the correct number of user posts', async () => {
      const res: Response = await request(server).get('/api/users/1/posts')
      expect(res.body).toHaveLength(initialPosts.filter((p: BasePost) => p.user_id == 1).length)
    })
    it('responds with a 404 if user id does not exist', async () => {
      const res: Response = await request(server).get('/api/users/111/posts')
      expect(res.status).toBe(404)
    })
  })
  describe('7 [POST] /api/users/:id/posts', () => {
    it('creates a new user post in the database', async () => {
      ...
      let posts: Post[] = await db('posts').where('user_id', 1)
      expect(posts).toHaveLength(initialPosts.filter((p: BasePost) => p.user_id == 1).length + 1)
      await request(server).post('/api/users/1/posts').send({ text: 'bar' })
      posts = await db('posts').where('user_id', 1)
      expect(posts).toHaveLength(initialPosts.filter((p: BasePost) => p.user_id == 1).length + 2)
    })
    it('responds with the newly created user post', async () => {
      let res: Response = await request(server).post('/api/users/1/posts').send({ text: 'foo' })
      expect(res.body).toHaveProperty('id')
      expect(res.body).toMatchObject({ text: 'foo' })
    })
    it('responds with a 404 if user id does not exist', async () => {
      let res: Response = await request(server).post('/api/users/111/posts').send({ text: 'foo' })
      expect(res.status).toBe(404)
    })
    it('responds with a 400 if missing text', async () => {
      let res: Response = await request(server).post('/api/users/1/posts').send({ no: 'foo' })
      expect(res.status).toBe(400)
    })
